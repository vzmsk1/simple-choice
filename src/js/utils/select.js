...
export class Select {
    ...
    setOptions(select, relativeSel) {
        ...
        setTimeout(() => {
            options.querySelectorAll(`.${this.classes.OPTION}`).forEach((option, indx) => {
                if (relativeSel.hasAttribute('data-sel-empty')) {
                    option.selected = false;
                    option.classList.remove(_this.classes.IS_SELECTED);
                }
                if (relativeSelOptions[indx].hasAttribute('selected')) {
                    option.classList.add(this.classes.IS_SELECTED);
                    select.classList.add(this.classes.IS_FILLED);
                }
            });
        }, 0);
    }
    ...
    getOption(option, relativeSel) {
        const selections = option.hasAselected ? ` ${this.classes.IS_SELECTED}` : '';
        const showSelection =
            option.selected && !relativeSel.hasAttribute('data-show-selection') && !relativeSel.multiple
                ? `hidden`
                : ``;
        const optionLang = option.dataset.language ? `data-language="${option.dataset.language.trim()}"` : '';
