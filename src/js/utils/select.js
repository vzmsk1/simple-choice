...
export class Select {
  ...
  setOptions(select, relativeSel) {
    ...
    setTimeout(() => {
      options
        .querySelectorAll(`.${this.classes.OPTION}`)
        .forEach((option, indx) => {
  ...
  getOption(option, relativeSel) {
    const selections = option.hasAselected
    ...
    const showSelection =
      ...
      !relativeSel.multiple
        ? `hidden`
        : ``;
    const optionLang = option.dataset.language
      ? `data-language="${option.dataset.language.trim()}"`
      : "";
